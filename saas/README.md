...
2. **Экран Super Admin** (список всех заведений, usage, блокировка/
   разблокировка через уже готовые `enableTenant`/`disableTenant`) —
   отдельный клиент или веб-панель, решить стек.
