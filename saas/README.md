...
2. **Управление сотрудниками из консоли**: сейчас консоль показывает
   только владельца/одно членство и код устройств — список сотрудников,
   приглашение по email, смена/отзыв роли (сама Cloud Function на роль
   `owner` уже запрещает эскалацию, см. `firestore.rules`, — не хватает UI).
3. **Экран Super Admin** (список всех заведений, usage, блокировка/
