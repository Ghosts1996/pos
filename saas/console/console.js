...
// в терминале заново.
const CONSOLE_BUILD = '2026-09-18.6';
  ...
  screenEl().innerHTML = `
    ...
    <div class="card" style="margin-top:6px">
      ...
      <label class="row" style="align-items:flex-start;gap:8px;margin-bottom:14px">
        ...
        <span class="small muted">Принимаю условия <a href="#/legal/offer" target="_blank" rel="noopener">публичной оферты</a> и даю согласие на обработку персональных данных согласно <a href="#/legal/privacy" target="_blank" rel="noopener">политике конфиденциальности</a></span>
      </label>
...
function screenLegalOffer() {
  screenEl().innerHTML = publicPageWrapHtml('Публичная оферта', `
    <p class="small muted" style="padding:10px 12px;background:var(--surface-2);border-radius:10px;border:1px solid var(--border)">
      ...
      (ст. 437 ГК РФ), и охватывает существенные условия. Перед реальной
      публикацией его обязательно должен проверить и подписать практикующий
      юрист — как минимум нужно: (1) вписать реквизиты Исполнителя (раздел 13),
      (2) подтвердить организационно-правовую форму (ИП / ООО / самозанятый —
      от неё зависят отдельные формулировки), (3) сверить раздел 11 (место
      разрешения споров) и применимость Закона РФ «О защите прав потребителей»
      к вашей категории заказчиков.
    </p>
    <div class="card">
      <h2 style="margin-top:0">1. Термины и определения</h2>
      <p class="small muted"><b>Исполнитель</b> — владелец и оператор Платформы (реквизиты — раздел 13). <b>Платформа</b> — программный комплекс Hookah POS, доступ к которому предоставляется через личный кабинет. <b>Заказчик</b> — лицо, акцептовавшее оферту в порядке раздела 2. <b>Тариф</b> — условия использования Платформы (лимиты, функции, стоимость), опубликованные в разделе «Тарифы» личного кабинета. <b>Заведение</b> — учётная единица Платформы (кальянная/лаунж/бар), созданная Заказчиком.</p>
    ...
    <div class="card">
      ...
      <p class="small muted">[Заполняется перед публикацией: полное наименование, ОГРН/ОГРНИП, ИНН, юридический/фактический адрес, банковские реквизиты, контактный email для претензий.]</p>
    </div>
  `);
}
...
function screenLegalPrivacy() {
  screenEl().innerHTML = publicPageWrapHtml('Политика обработки персональных данных', `
    <p class="small muted" style="padding:10px 12px;background:var(--surface-2);border-radius:10px;border:1px solid var(--border)">
      ...
      данных». Перед реальной публикацией его должен проверить юрист — включая
      раздел 7 ниже, где прямо указан открытый вопрос с местом хранения данных,
      и раздел «Реквизиты», где нужно вписать оператора и номер в реестре
      Роскомнадзора.
    </p>
    <div class="card">
      <h2 style="margin-top:0">1. Общие положения</h2>
      <p class="small muted">Оператором персональных данных является Исполнитель — владелец Платформы Hookah POS (реквизиты — раздел 12). Использование Платформы означает согласие субъекта персональных данных с условиями настоящей Политики.</p>
    ...
    <div class="card" style="border-color:var(--warning)">
      ...
      <p class="small muted">Инфраструктура Платформы размещена на облачной платформе Google Firebase / Google Cloud. <b>Открытый вопрос, требующий проверки перед публикацией:</b> необходимо точно установить регион(ы) физического расположения серверов, на которых размещены база данных Firestore и файловое хранилище, и соотнести это со ст. 18 ч. 5 152-ФЗ.</p>
      <p class="small muted">Согласно ст. 18 ч. 5 152-ФЗ, запись, систематизация, накопление, хранение, уточнение и извлечение персональных данных граждан РФ должны вестись с использованием баз данных, физически находящихся на территории РФ. Инфраструктура Google Cloud не имеет региона размещения в РФ ни в одном тарифе — если фактическое хранение данных происходит за пределами РФ, использование Платформы для обработки персональных данных граждан РФ на коммерческой основе сопряжено с риском для Оператора, вплоть до ограничения доступа к сервису по решению Роскомнадзора (ст. 15.5 ФЗ «Об информации...»).</p>
      <p class="small muted">Отдельно от требования локализации, трансграничная передача данных в государства, не входящие в перечень обеспечивающих адекватную защиту прав субъектов (утверждается Роскомнадзором), требует отдельного явного согласия субъекта на трансграничную передачу (ст. 12 152-ФЗ) — общего согласия на обработку персональных данных для этого недостаточно.</p>
      <p class="small muted">До устранения этого несоответствия (переноса основного хранения данных на инфраструктуру в РФ либо получения надлежащих правовых оснований) настоящий пункт не может считаться приведённым в соответствие с законодательством — решение остаётся за Оператором и его юристом.</p>
    </div>
    <div class="card">
    ...
    <div class="card">
      ...
      <p class="small muted">[Заполняется перед публикацией: полное наименование Оператора, ИНН/ОГРН, адрес, номер в реестре операторов персональных данных Роскомнадзора, email для обращений по вопросам персональных данных.]</p>
    </div>
  `);
}
