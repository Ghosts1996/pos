...
function screenLegalPrivacy() {
  screenEl().innerHTML = publicPageWrapHtml('Политика обработки персональных данных', `
    <p class="small muted" style="padding:10px 12px;background:var(--surface-2);border-radius:10px;border:1px solid var(--border)">
      ...
      раздел 7 ниже: явное согласие на трансграничную передачу теперь запрашивается
      при регистрации, но требование о локализации первичного хранения данных
      (ст. 18 ч. 5 152-ФЗ) этим НЕ снимается и остаётся открытым архитектурным
      вопросом. Также нужно заполнить раздел «Реквизиты» — оператора и номер
