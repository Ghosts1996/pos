...
///
/// ЧЕСТНО про то, что здесь реально работает, а что нет — по провайдерам:
///
///   • [ManualTerminalService] — по-настоящему рабочий вариант для ЛЮБОГО
///     физического терминала: Ingenico, Verifone, банковский mPOS-ридер,
///     фирменный терминал Сбера/Тинькофф/ВТБ/Альфы/Точки — вообще любого,
///     который стоит в заведении сам по себе. Software-интеграции с этими
///     терминалами (когда касса сама "толкает" сумму в терминал по кабелю/
///     Bluetooth) не бывает без официального SDK банка, который выдаётся
///     только после заключения договора эквайринга на конкретное
///     юрлицо — просто кодом это не обойти, и выдавать желаемое за
///     действительное здесь не будем. Поэтому кассир вбивает сумму в
///     терминал сам, как во множестве заведений уже происходит, а
///     приложение просто фиксирует итог операции по слипу терминала.
///
///   • [TinkoffSbpQrTerminalService] — по-настоящему рабочая интеграция:
///     публичное REST API Т-Банка (Интернет-эквайринг, oplata.tinkoff.ru)
///     позволяет выставить счёт и получить QR СБП без физического
///     терминала вообще — гость сканирует код своим банковским
///     приложением и платит сам. Нужны только TerminalKey и пароль,
///     которые банк выдаёт после регистрации на сайте — никакого
///     проприетарного SDK. Точные названия полей ответа стоит сверить с
///     документацией банка перед боевым использованием (см. комментарии
///     внутри класса) — открытого Swagger нет, методичку иногда уточняют.
///
///   • Сбербанк/ВТБ/Альфа-Банк/Точка Банк, mPOS, "чистые" Ingenico/
///     Verifone-SDK — под них ниже оставлены классы-заготовки с нужными
///     полями настроек (Настройки → Интеграции их сохранят), но сам HTTP-
///     или SDK-вызов не реализован: у каждого банка свой протокол,
///     видимый только после подписания договора эквайринга, а угадывать
///     его вместо чтения реальной документации значило бы дать вам код,
///     который выглядит рабочим, а на первой же настоящей оплате откажет
///     или, хуже, спишет не ту сумму. Как только у вас будет договор с
///     конкретным банком — пришлите его техническую документацию, и здесь
///     дописывается один класс, аналогичный Т-Банку выше.
